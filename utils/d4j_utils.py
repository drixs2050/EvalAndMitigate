...
def _compile_and_collect_results(root):
    ...
    try:
        ...
        compile_error_lines = compile_proc.stderr.decode("latin-1").split("\n")[2:]
    compile_error_lines = [
        e
...
def check_compile(bug_id):
    ...
    if os.path.exists(fixed_dir) and os.path.exists(buggy_dir):
        ...
        buggy_pass, buggy_err_info, buggy_errors = _compile_and_collect_results(buggy_dir)
        return {
            "fixed_pass": fixed_pass,
            "fixed_error_info": fixed_err_info,
            "fixed_errors": fixed_errors,
            "buggy_pass": buggy_pass,
            "buggy_error_info": buggy_err_info,
            "buggy_errors": buggy_errors,
        }
    else:
...
def check_test(bug_id, test_class_sig, report_dir, buggy_pass):
    fixed_dir = os.path.join(d4j_proj_base, f"{bug_id}/fixed")
    buggy_dir = os.path.join(d4j_proj_base, f"{bug_id}/buggy")
    if os.path.exists(fixed_dir) and os.path.exists(buggy_dir):
        jacoco_error = None
        ...
        try:
            fixed_coverage = _check_coverage(fixed_dir, bug_id, report_dir)
        except JacocoFailedException as e:
            fixed_coverage = {}
            jacoco_error = e
        fixed_err_report = fixed_res["error_report"]
        del os.environ["JAVA_TOOL_OPTIONS"]
        if buggy_pass:
            ...
            os.system(f"rm {bz_file_path}")
            buggy_passed = buggy_res["passed"]
            buggy_err_info = buggy_res["error_info"]
            buggy_err_types = buggy_res["error_types"]
            buggy_err_report = buggy_res["error_report"]
        else:
            buggy_passed = buggy_pass
            buggy_err_info = []
            buggy_err_types = []
            buggy_err_report = ''
    ...
    return {
        ...
        "fixed_error_types": fixed_err_types,
        "fixed_error_info": fixed_err_info,
        "fixed_error_report": fixed_err_report,
        "buggy_passed": buggy_passed,
        "buggy_error_types": buggy_err_types,
        "buggy_error_info": buggy_err_info,
        "buggy_error_report": buggy_err_report,
        "jacoco_error": jacoco_error
    }
    pass

def check_test_buggy(bug_id, test_class_sig, report_dir, fixed_pass):
    fixed_dir = os.path.join(d4j_proj_base, f"{bug_id}/fixed")
    buggy_dir = os.path.join(d4j_proj_base, f"{bug_id}/buggy")
    if os.path.exists(fixed_dir) and os.path.exists(buggy_dir):
        jacoco_error = None
        ...
        buggy_err_types = buggy_res["error_types"]
        try:
            buggy_coverage = _check_coverage(buggy_dir, bug_id, report_dir)
        except JacocoFailedException as e:
            buggy_coverage = {}
            jacoco_error = e
        buggy_err_report = buggy_res["error_report"]
...
def _compress_bz_files(bug_id, version, test_class_sig):
    ...
    if not os.path.exists(test_file):
        ...
        return None
    subprocess.run(
        f"tar -cjf LLMGeneratedTests.tar.bz {test_file}",
        shell=True,
