...
def compile_and_test(
    ...
    if compile_res["fixed_pass"]:
        # 获得测试结果
        execute_res = check_test(bug_id, test_class_sig,jacoco_report_base, compile_res["buggy_pass"])
        pass
    else:
        execute_res = None

    if execute_res is not None:
        if execute_res.get("jacoco_error") is not None:
            error_msg = execute_res.get("jacoco_error")

    result = _analyze_compile_res(compile_res, execute_res, test_class_content, round)
    result["JacocoFailedException"] = error_msg
    # result['execute_res'] = execute_res

    return result

def compile_and_test_buggy(
        bug_id,
        test_class_sig,
        test_class_content,
        focal_method_name,
    ...
    jacoco_report_base = os.path.join(
        code_base,
        f"rq1/coverage_reports/{compile_level}/buggy/{model}_{strategy}_{ablation}_{format}/{round}/{bug_id}_{focal_method_name}",
    )
    error_msg = None
    if compile_res["buggy_pass"]:
        # 获得测试结果
        execute_res = check_test_buggy(bug_id, test_class_sig,jacoco_report_base, compile_res["fixed_pass"])
        pass
    else:
        execute_res = None
    ...
    result = _analyze_compile_res_buggy(compile_res, execute_res, test_class_content, round)
    result["JacocoFailedException"] = error_msg
    # result['execute_res'] = execute_res
    ...
    if compile_res["fixed_pass"]:
        ...
        if len(failed_imports) != 0:
            ...
            ret_dict = {
                ...
                "buggy_compiled": compile_res["buggy_pass"]
            }
            return ret_dict
        else:
...
def _analyze_compile_res_buggy(compile_res, execute_res, test_class_content, round):
    methods = parse_methods_from_class_node(test_class_content, False)
    uts = [method for method in methods if "@Test" in method["method_modifiers"]]
    total_methods_num = len(methods)
    total_uts_num = len(uts)
    ...
    if compile_res["buggy_pass"]:
        ...
        # 筛选出编译错误中出现错误的行号
        error_line_idxes, err_types = get_sorted_error_line_num(
            error_str=compile_res["buggy_error_info"]
        )
        ...
        if len(failed_imports) != 0:
            ...
            ret_dict = {
                "compiled": compiled,
                "err_msg": compile_res["buggy_error_info"],
                "err_types": err_types,
                "msg": msg,
                "total_methods_num": total_methods_num,
                "total_uts_num": total_uts_num,
                ...
                "coverage_results": coverage_results,
                "failed_methods": methods,
                "fixed_compiled": compile_res["fixed_pass"]
            }
            return ret_dict
        else:
            # 如果没有import出错，筛选出现编译错误的函数，重新组合测试类，进行第二次编译
...
def get_sorted_error_line_num(error_str):
    ...
    """
    pattern = r"\[.*\] .+\/d4j\_proj\_base\/.*\.java:(\d+): error: (.*)"
    error_line_list = []
